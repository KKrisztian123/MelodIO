...
export class SongService {
  ...
  async createSong(songData, token): Promise<string | true> {
    ...
    const [res] = await this.conn.execute(
      'INSERT INTO `song` (songName, songType, fileName, albumId, token) VALUES (?,?,?,?,?)',
      [
        songData.songName,
        songData.fileType,
        songData.fileName,
  ...
  async getLikedSongs(authHeader, protocol, host) {
    ...
    const [albums] = await this.conn.execute(
      'SELECT album.id, album.name, album.token, album.imageURL, album.imageType, album.albumType, album.releaseDate FROM `likedSong` INNER JOIN `song` ON song.id = likedSong.songId INNER JOIN `album` ON album.id = song.albumId WHERE userId = ?',
      [currentUser.userId],
    );
  ...
  async likeSong(authHeader, songId, value: boolean) {
    ...
    if (value) {
      await this.conn.execute(
        ...
        [songInfo[0].id, currentUser.userId],
      );
      return value;
    }
    await this.conn.execute(
  ...
  async updateSong(id, songData) {
    ...
    if (albumInfo.length === 0) return 'Nincs ilyen album!';
    await this.conn.execute(
      'UPDATE `song` SET songName = ?, songType = ?, albumId = ?, fileName = ? WHERE id = ?',
      [
        songData.songName,
        songData.fileType,
        albumInfo[0].id,
  ...
  async getSong(songId: string) {
    const [res] = await this.conn.execute(
      'SELECT song.id, song.token AS `songToken`,song.fileName, album.token, song.songName, song.songType FROM `song` INNER JOIN `album` ON album.id = song.albumId WHERE song.token = ?',
      [songId],
    );
